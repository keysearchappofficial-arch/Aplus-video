(function () {
  ...
  const articles = [
    ...
    makeArticle({
      slug: "srt-multichannel-audio-obs-troubleshooting",
      relatedTool: null,
      relatedProduct: null,
      category: "專業製作",
      title: "SRT 明明有 4 聲道，OBS 為什麼看起來只有一軌？我的實際排查方式",
      description: "一次跨地直播測試中，對方明確告訴我 SRT 訊號包含 4 個 Audio Channel，但進到 OBS 後卻像只剩下一組 Stereo。這篇整理我後來怎麼把 Audio Stream、Channel 與 OBS Track 分開判斷，以及現在再遇到同樣問題時，我會怎麼查。",
      lead: "SRT 有聲音，不代表你真的收到了對方送來的所有音訊。更麻煩的是，OBS 裡看到「一條音訊」，也不一定代表來源真的只有兩個 Channel。這是我最近實際測試多聲道 SRT 時，花了一段時間才釐清的地方。",
      date: "2026.08.18",
      readingTime: "6 min read",
      image: "./assets/images/solutions/corporate-live.jpg",
      alt: "SRT 四聲道音訊進入 OBS 後顯示為立體聲的多聲道音訊排查示意",
      seoTitle: "SRT 4 聲道進 OBS 為什麼只剩 Stereo？多聲道音訊排查實測",
      metaDescription: "SRT 明明送了 4 個 Audio Channel，OBS 卻只看到一組 Stereo？從實際跨地直播測試出發，整理 Audio Stream、Channel、OBS Track 的差別，以及用 ffprobe 判斷多聲道訊號的排查方法。\n\n這篇的技術基礎有另外核對過：OBS 官方目前說明可處理最多 8 個 Audio Channel，且其 Multiple Audio Track 是另一套輸出／錄製分軌概念；OBS 的 SRT Guide 也確認 SRT 可透過其 FFmpeg/MPEG-TS 路徑使用。FFmpeg 官方則明確區分輸入 Stream 與輸出時的 Stream Selection／Mapping。\n\n另外，SRT 官方文件也再次強調 Caller／Listener 描述的是連線建立方式，並不等同影音的 Source／Destination，這也是排查時不應把「誰 Caller」直接當成「誰送訊號」的原因。",
      intro: "一次跨地直播測試中，對方明確告訴我 SRT 訊號包含 4 個 Audio Channel，但進到 OBS 後卻像只剩下一組 Stereo。這篇整理我後來怎麼把 Audio Stream、Channel 與 OBS Track 分開判斷，以及現在再遇到同樣問題時，我會怎麼查。",
      sections: [],
      extra: "<p>我最近在做一次跨地直播測試時，碰到一個一開始很容易判斷錯的問題。</p>\n\n<p>對方很明確告訴我，送過來的訊號裡面有 <strong>4 個 Audio Channel</strong>。前兩個 Channel 是節目聲音，另外兩個則是另一組可以獨立使用的聲音。但訊號進到 OBS 之後，我看到的狀況卻很像只有一組 Stereo。</p>\n\n<p>更麻煩的是，聲音不是完全沒有。我確實聽得到主持、講評與現場聲音，所以第一個直覺很容易變成：「那應該就是對方已經全部 Mix 在一起了。」</p>\n\n<p>後來實際往下查，我才發現這個判斷不能只靠耳朵，也不能只看 OBS Mixer。</p>\n\n<h2>我一開始搞混的是三件不同的東西</h2>\n\n<p>這次測試之後，我現在看到多聲道問題，第一件事反而不是去調 OBS，而是先把三個概念拆開：</p>\n\n<p><strong>Audio Stream、Audio Channel、OBS Track。</strong></p>\n\n<p>這三個東西看起來都跟「音軌」有關，但實際上不是同一層。</p>\n\n<p>一個影音來源可以包含一個或多個 Audio Stream；而一個 Audio Stream 裡面，又可能是 Mono、Stereo、4CH、5.1 或其他 Channel Layout。</p>\n\n<p>OBS 裡面設定的 Track 1、Track 2、Track 3，則又是 OBS 自己在錄製或輸出階段使用的音訊軌道概念。OBS 官方文件本身也把多 Track 錄製與多 Channel 音訊分開處理。</p>\n\n<blockquote>所以看到 OBS 裡只有一個 Audio Source，不能直接推論來源只有兩個 Channel；反過來，來源有 4CH，也不代表 OBS 一定會自動幫你拆成四條可以單獨控制的 Mixer。</blockquote>\n\n<h2>第一步不是聽，是先確認來源到底送了什麼</h2>\n\n<p>如果現在再遇到一次同樣的問題，我第一步會先跳過 OBS 的操作介面，直接檢查輸入訊號本身。</p>\n\n<p>我後來在做自己的 Decoder 時，也把這件事情放得很前面：Analyze Source 之後，要讓軟體先告訴我來源到底有多少 Audio Stream、每一個 Stream 有多少 Channel，而不是固定假設來源一定是 Stereo 或固定四聲道。</p>\n\n<p>用 FFmpeg 工具來看，可以先用 ffprobe 檢查：</p>\n\n<pre><code>ffprobe \"srt://0.0.0.0:6010?mode=listener\"</code></pre>\n\n<p>實際的 IP、Port 與 Caller／Listener 模式要依現場架構修改。</p>\n\n<p>我真正想看的不是「有沒有聲音」，而是 ffprobe 列出來的 Audio Stream 資訊。例如它到底只有一個 Stereo Audio Stream，還是一個包含多個 Channel 的 Audio Stream，又或者其實存在 Audio Stream 1、Audio Stream 2。</p>\n\n<p>這一步很重要，因為如果來源本身進來就只有兩個 Channel，那後面再怎麼調 OBS 都不會憑空變出另外兩個 Channel。</p>\n\n<h2>有聲音，不代表四個 Channel 都還在</h2>\n\n<p>這也是我這次最容易被誤導的地方。</p>\n\n<p>當時我確實可以聽到主持、講評和現場聲音。從使用者角度來看，會覺得「需要的聲音都有，那應該代表四軌都收到了」。</p>\n\n<p>但這兩件事情其實不能畫上等號。</p>\n\n<p>假設來源端把不同內容先 Mix 到同一組 Stereo，接收端一樣可以把所有東西聽得很清楚，但原本獨立的 Channel 已經沒有辦法分開使用。</p>\n\n<p>相反地，也有可能 Transport Stream 裡面的多聲道資料還在，只是目前使用的播放或接收軟體沒有按照我想要的方式把它攤開。</p>\n\n<p>所以現在我的判斷方式會變成：</p>\n\n<p><strong>先確認封包裡有什麼，再確認軟體怎麼呈現，最後才去判斷是哪一端把聲音合併了。</strong></p>\n\n<h2>OBS 的 Track 1 到 6，不是來源的 CH1 到 CH6</h2>\n\n<p>這一點也是我實際操作 OBS 時一度搞混的地方。</p>\n\n<p>OBS 的 Advanced Audio Properties 裡，可以看到 Track 1、2、3、4、5、6，而且每個 Audio Source 都可以決定送到哪些 Track。</p>\n\n<p>但這些 Track 比較接近 OBS 內部的輸出分配。</p>\n\n<p>例如麥克風可以送 Track 1 和 Track 2，現場聲音只送 Track 1，錄影時再把不同 Track 分開保存。OBS 官方也有專門針對 Multiple Audio Track Recording 的設定方式。</p>\n\n<p>這不代表一個透過 SRT 進來的 4CH Audio Stream，就會自動在這裡變成 Track 1、2、3、4。</p>\n\n<blockquote>我現在會把它理解成兩個階段：先把來源的 Channel 正確拆出來，再決定這些聲音要送進 OBS 的哪一個 Track。不要把這兩個步驟混在一起。</blockquote>\n\n<h2>那為什麼 OBS Mixer 看起來還是只有 L／R？</h2>\n\n<p>OBS 本身並不是只能處理兩個 Audio Channel。官方的 Surround Sound 說明中，目前可以處理最多 8 個 Audio Channel。</p>\n\n<p>但「OBS 支援多聲道」跟「任何 SRT 輸入進來都會自動變成多個獨立 Mixer」是兩回事。</p>\n\n<p>實際上還牽涉到來源封裝方式、Audio Stream 結構、FFmpeg 解封裝，以及 OBS 使用 Media Source 接收後怎麼把這組音訊暴露給 Mixer。</p>\n\n<p>所以如果我的目標只是確認 SRT 有沒有聲音，OBS 很方便；但如果我要知道對方到底送了幾個 Stream、每個 Stream 幾個 Channel，以及我要精準選擇其中哪一個 Channel，我現在不會只靠 OBS 判斷。</p>\n\n<h2>這也是我後來想做 Decoder 的原因之一</h2>\n\n<p>這次碰到問題之後，我其實一直在想：如果每次都要另外開 ffprobe、看一串文字，再回到 OBS 試聽，對現場工作來說還是太繞。</p>\n\n<p>所以我後來在做 Decoder 時，對 Audio Source 的想法很明確。</p>\n\n<p>接收到 SRT 之後，第一步先 Analyze，直接告訴我：</p>\n\n<p>來源裡面有幾個 Audio Stream、每個 Stream 有幾個 Channel，然後讓我決定哪些 Channel 要進 Program、哪些只做 Local Monitor。</p>\n\n<p>例如來源真的有 CH1、CH2、CH3、CH4，我希望可以先分別監聽，再決定最後送 CH1/2、CH3/4，或全部送出，而不是只給我一個音量 Meter 然後讓我猜。</p>\n\n<p>對我來說，這不是為了把 Decoder 做得功能很多，而是因為實際排錯時，我真正缺的就是這一層資訊。</p>\n\n<h2>現在我會怎麼排查</h2>\n\n<p>如果今天又有人跟我說：「我們送了 4CH，但你那邊怎麼只有 Stereo？」我現在不會先回答是 OBS 的問題，也不會先叫對方改設定。</p>\n\n<p>我會從訊號來源一路往後查。</p>\n\n<p>第一個先確認對方所說的「4CH」到底是四個 Mono Channel、一個 4CH Stream，還是兩組 Stereo Stream。</p>\n\n<p>第二個直接檢查收到的 Transport Stream。只要這裡還能看到完整音訊結構，就代表 SRT 傳輸本身至少沒有把那些 Audio Stream 憑空吃掉。</p>\n\n<p>第三個才進 OBS，看 Media Source 最後怎麼呈現這些音訊。</p>\n\n<p>如果 ffprobe 看得到、OBS 卻沒辦法按照需求選擇，我就知道問題已經從「網路傳輸」縮小到「接收軟體怎麼處理多聲道」。</p>\n\n<p>如果 ffprobe 一開始看到的就不是預期的結構，那就應該往 Encoder、Mux 或來源設備查，而不是一直改 OBS。</p>\n\n<h2>SRT 本身不是決定你有幾軌聲音的東西</h2>\n\n<p>這也是我後來比較重要的一個理解。</p>\n\n<p>SRT 處理的是資料怎麼可靠地從一端傳到另一端。實際傳輸的影音通常還會有 MPEG-TS 等封裝，裡面才包含 Video Stream、Audio Stream 等資料。</p>\n\n<p>所以「使用 SRT」本身並不等於「一定只有 Stereo」，也不等於「一定可以任意拆成多軌」。</p>\n\n<p>真正要看的還是來源怎麼編碼、怎麼封裝，以及接收端怎麼 Demux 和解碼。</p>\n\n<p>FFmpeg 在處理多 Stream 輸出時，也會區分自動 Stream Selection 與手動 Mapping。這也是為什麼我在做 Encoder／Decoder 時，越來越不想把 Audio Routing 做成固定值。</p>\n\n<h2>我現在最在意的是「不要太快認定是哪裡壞了」</h2>\n\n<p>以前看到 OBS 只有一組 L／R，我可能第一時間就會覺得：「對方是不是只送 Stereo？」</p>\n\n<p>經過這次測試後，我反而覺得這種問題最容易出錯的地方，就是太快根據某一個軟體的 UI 判斷整條訊號。</p>\n\n<p>傳輸端說 4CH、Transport Stream 裡有 4CH、Decoder 解出 4CH，以及 OBS 最後讓你控制 4CH，是四件需要分開確認的事情。</p>\n\n<p>現在如果我要確認多聲道 SRT，我會先把來源結構看清楚，再決定下一步。</p>\n\n<p><strong>先確認訊號，再確認軟體；先確認 Stream，再處理 Routing。</strong></p>\n\n<p>這樣做看起來多了一個步驟，但比起在 OBS 裡一直試設定，反而快很多。</p>",
    })
  ];
